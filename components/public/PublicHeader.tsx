  ...
  return (
    <>
      <style>{`
        ...
        .pub-header-inner{display:flex;align-items:center;justify-content:space-between;padding:18px 28px;max-width:1120px;margin:0 auto;}
        ...
        .pub-nav{display:flex;gap:18px;align-items:center;}
        .pub-nav a{font-family:'JetBrains Mono',monospace;font-size:10.5px;letter-spacing:0.07em;text-transform:uppercase;color:var(--muted);transition:color .2s;white-space:nowrap;}
        ...
        @media(max-width:900px){.pub-nav{display:none;}.pub-hamburger{display:block;}}
        .pub-mobile-menu{display:none;position:fixed;top:0;left:0;right:0;bottom:0;background:rgba(5,5,5,0.97);z-index:30;flex-direction:column;align-items:center;justify-content:center;gap:28px;}
      ...
      <div className={`pub-mobile-menu${mobileOpen ? ' open' : ''}`}>
        <button className="pub-mobile-close" onClick={() => setMobileOpen(false)} aria-label="Close menu">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><line x1="18" y1="6" x2="6" y2="18" /><line x1="6" y1="6" x2="18" y2="18" /></svg>
        </button>
        {NAV_LINKS.map((link) => <Link key={link.href} href={link.href} onClick={() => setMobileOpen(false)}>{link.label}</Link>)}
      </div>
    </>
  );
}
