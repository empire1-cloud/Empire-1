...
class SonicForgeCore:
    """
    Sonic Forge — Vertex AI music composition engine.
    Generates JSON composition briefs and SFX specs for downstream synthesis.
    """
...
Return JSON with: bpm, key, time_signature, instruments[], layers[], mood, fx_chain[], mix_notes, Southern_canon_tags[], production_directives."""
        ...
        try:
            response = model.generate_content(
                [Part.from_text(self.SYSTEM_PROMPT), Part.from_text(composition_prompt)],
    ...
    async def generate_sfx(self, scene: str, intensity: str = "medium") -> dict:
        ...
        model = GenerativeModel(settings.VERTEX_AI_TEXT_MODEL)
        sfx_prompt = (
            f"Generate SFX design spec for: {scene}. Intensity: {intensity}. "
            "Return JSON with: sfx_layers[], trigger_events[], spatial_mix, reverb_profile, arcade_zone."
        )
        try:
            response = model.generate_content(
                [Part.from_text(self.SYSTEM_PROMPT), Part.from_text(sfx_prompt)],
