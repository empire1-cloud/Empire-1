...
from ..engines.arcade.fishing_engine_v2 import FishingEngineV2
from ..engines.arcade.slots_engine_v1 import SlotsEngineV1
from ..engines.arcade.keno_engine_v1 import KenoEngineV1
from ..engines.arcade.payout_engine import PayoutEngine
from ..engines.arcade.machine_engine import MachineEngine
from ..engines.arcade.tenant_engine import TenantEngine
from ..engines.arcade.canon_layer_sgv_aztec import CanonLayerSGVAztec
from ..services.tenant_access import PermissionChecker
...
def _check_access(tenant_id: str, engine_id: str):
    if not PermissionChecker.check_engine_access(tenant_id, engine_id):
        raise HTTPException(
            status_code=403,
            detail=f"Tenant '{tenant_id}' does not have access to engine '{engine_id}'"
        )
...
def fish_spin(
    ...
    if body.apply_canon and PermissionChecker.check_engine_access(tenant_id, "canon_layer_sgv_aztec"):
...
def slots_spin(
    ...
    if body.theme_override and hasattr(engine, "theme"):
        from ..engines.arcade.slots_engine_v1 import VALID_THEMES
...
def apply_canon_layer(
    ...
    tenant_id = _require_tenant(x_tenant_id)
    _check_access(tenant_id, "canon_layer_sgv_aztec")
